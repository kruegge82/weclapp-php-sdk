...
class DocumentApi
{
    ...
    public function documentCopyPostWithHttpInfo($document_copy_post_request, string $contentType = self::contentTypes['documentCopyPost'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\kruegge82\weclapp\Model\DocumentCopyPost200Response' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\kruegge82\weclapp\Model\DocumentCopyPost200Response' !== 'string') {
                            try {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\kruegge82\weclapp\Model\DocumentCopyPost200Response', []),
    ...
    public function documentIdIdGetWithHttpInfo($id, string $contentType = self::contentTypes['documentIdIdGet'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\kruegge82\weclapp\Model\Document' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\kruegge82\weclapp\Model\Document' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\kruegge82\weclapp\Model\Document', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                default:
