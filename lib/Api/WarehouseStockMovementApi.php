...
class WarehouseStockMovementApi
{
    ...
    public function warehouseStockMovementBookDirectStockTransferPostWithHttpInfo($warehouse_stock_movement_book_direct_stock_transfer_post_request, string $contentType = self::contentTypes['warehouseStockMovementBookDirectStockTransferPost'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\kruegge82\weclapp\Model\WarehouseStockMovementGet200Response' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\kruegge82\weclapp\Model\WarehouseStockMovementGet200Response' !== 'string') {
                            try {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\kruegge82\weclapp\Model\WarehouseStockMovementGet200Response', []),
    ...
    public function warehouseStockMovementIdIdGetWithHttpInfo($id, string $contentType = self::contentTypes['warehouseStockMovementIdIdGet'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\kruegge82\weclapp\Model\WarehouseStockMovement' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\kruegge82\weclapp\Model\WarehouseStockMovement' !== 'string') {
                            try {
                                $content = json_decode($content, false, 512, JSON_THROW_ON_ERROR);
                            } catch (\JsonException $exception) {
                                throw new ApiException(
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\kruegge82\weclapp\Model\WarehouseStockMovement', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
                default:
