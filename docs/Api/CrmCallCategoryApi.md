...
$apiInstance = new kruegge82\weclapp\Api\CrmCallCategoryApi(
...
try {
    $apiInstance->crmCallCategoryIdIdDelete($id, $dry_run);
} catch (Exception $e) {
    echo 'Exception when calling CrmCallCategoryApi->crmCallCategoryIdIdDelete: ', $e->getMessage(), PHP_EOL;
}
```
...
try {
    $result = $apiInstance->crmCallCategoryIdIdGet($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling CrmCallCategoryApi->crmCallCategoryIdIdGet: ', $e->getMessage(), PHP_EOL;
}
```
